...
export const login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(
            { userId: user.account_id, email: user.email }, // Payload
        ...
        // Respond with success message and account_id
        res.json({
            message: 'Login successful',
            token, // Include the token in the response
            account_id: user.account_id // Include only the account_id
        });
    } catch (error) {
        res.status(500).json({ message: 'Server error', error: error.message });
    }
...
//     try {
//         // const [results] = await db.execute('SELECT name, email, account_type FROM accounts WHERE account_id = ?', [account_id]);
//         const [results] = await db.execute('SELECT name, email, account_type FROM accounts WHERE account_id = ?', [account_id]);
